...
def main(input_path: Path):
    ...
    orig_list = [Val(int(s)*E_KEY) for s in in_str.splitlines()]

    # for v in orig_list:
    #     v.value = (v.value)#%len(orig_list)

    zero_idx = [i for i,v in enumerate(orig_list) if v.value==0][0]
    mixed_list = []
    for i,v in tqdm(enumerate(orig_list)):
    ...
    logging.debug("Final Coords:")
    logging.debug(str(coords) + "\n")
    return sum(coords)
