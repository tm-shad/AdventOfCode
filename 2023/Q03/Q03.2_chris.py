...
for sym in symbols:
    ...
    for i in range(-1, 2):
        for j in range(-1, 2):
            ...
            print("num", input_text[x][start_coords[1]:end_coords[1]+1])
            # nums.append(int(input_text[x][start_coords[1]:end_coords[1]+1]))

            sym_coords.add((start_coords, end_coords))
    coords.append(sym_coords)

nums = []
for sym_coords in coords:
    if len(sym_coords) != 2:
        pass
    else:
        s = 1
        for start_coords, end_coords in sym_coords:
            print(input_text[start_coords[0]][start_coords[1]:end_coords[1]+1])
            s *= int(input_text[start_coords[0]][start_coords[1]:end_coords[1]+1])
        nums.append(s)
